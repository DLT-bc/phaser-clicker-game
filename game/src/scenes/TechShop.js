...
export class TechShop extends Phaser.Scene {
    ...
    create (data) {
        ...
        this.techPrice = this.add.text(TechShop.WIDTH * 0.2, TechShop.HEIGHT * 0.4,
            `${localization.getLocale('techShop1')}: ${techInfo[this.userData.techLvl].price}`, 
            {
               fontSize: '25px',
               fontFamily: 'Montserrat',
               color: '#ffffff',
    ...
    updateInfo() {
        ...
        this.techPrice.setText(`${localization.getLocale('techShop1')}: ${techInfo[this.userData.techLvl].price}`)
    }
